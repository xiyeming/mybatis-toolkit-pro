...
- **连接弹窗日志与交互**：取消按钮正确关闭 WebView 窗口；测试连接按钮状态恢复与结果回传添加 try-catch 保护；`handleTestConnection` 与 `DatabaseService.testConnection()` 补充全链路日志（参数、适配器类型、耗时、结果），日志输出到 **MyBatis Connection** 输出通道。
- **代码生成弹窗交互**：取消按钮正确关闭窗口；生成按钮支持 loading 状态（禁用 + 文字变化）；成功后自动关闭弹窗；失败时在弹窗内显示红色错误信息，不阻塞后续操作。
